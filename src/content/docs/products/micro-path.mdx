...
description: Rotary glide path files for endodontics, including Micro-Path Files, clinical rationale for glide path preparation, size progression, and links to motor settings before shaping
---
...
[Micro-Path Files](../../products/micro-path/) are EndoTech's dedicated rotary glide path instruments.
...
Micro-Path uses **Transform Metal Technology** to support a more flexible response during negotiation, especially where canal curvature becomes less forgiving.

## Practical Sequence Logic

The Micro-Path range keeps the glide path message simple:
...
## How Micro-Path Links Into ET Technique

In the current documented ET workflow, **Micro-Path 13/.03** is positioned as the opening glide path file before ET shaping begins.
